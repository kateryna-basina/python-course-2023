...
result = overovani_cisla(cislo_uzivatele) #estli uzivatel zadal nespravne cislo, vrati mu spravu,ze cislo je nespravne, a pak program ukonci
if not result:
    ...
    pocet_blokov = (len(sms) // 180)  #spocitame kolik bloku ze 180 znaku mame v sms
    def spocitat_cenu (sms): #funkce na zaklade pocetu bloku spocita cenu sms
        if len(sms) % 180 ==  0:
